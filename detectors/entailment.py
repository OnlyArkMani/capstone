...
def entailment_scores(
    ...
    for i, d in enumerate(docs):
        evidence = doc_text(d)
        probs = b.score(evidence, generated_claim) if evidence.strip() else {
...
def pairwise_conflict(
    ...
    for (i, a), (j, bdoc) in itertools.combinations(enumerate(docs), 2):
        ...
        if not ta.strip() or not tb.strip():
            continue
        c = max(b.score(ta, tb)["contradiction"], b.score(tb, ta)["contradiction"])
        ida = a.get("doc_id", f"doc_{i}")
        idb = bdoc.get("doc_id", f"doc_{j}")
